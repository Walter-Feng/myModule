...
def band_eigenvalues_list(band_path_dict):
    tot_number = len(band_path_dict['sample_0']["k_channel_0"]['density_re'])
    n_samples = band_path_dict['n_samples']

    eigenvalues = np.array([band.np_array_fill_math_inf(
        np.array(band_path_dict['sample_'+str(i)]["k_channel_0"]['eigenvalues']),tot_number) for i in range(n_samples)])
...
def file_full_extract(json_dir, keyword):
    with open(json_dir, 'r') as f:
        temp = f.read()
        inp = json.loads(temp.replace("'", "\"").replace("True", "true").replace("False", "false"))

        return named_structure_to_full_bands(inp[keyword])
