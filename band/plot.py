...
def full_bands_add_plot_all(full_bands_object,*args,**kwargs):
    ...
    for i in range(full_bands_object.length()):
        for j in range(full_bands_object.n_evals()):
            ...
            plt.plot(full_bands_object.x_array[i],temp[:,j],full_bands_object.colors_list[i][j],*args, **kwargs)
...
def full_bands_add_sub_plot_all(subplot,full_bands_object,*args,**kwargs):
    ...
    for i in range(full_bands_object.length()):
        for j in range(full_bands_object.n_evals()):
            temp = full_bands_object.get_evals_list()[i]
            subplot.plot(full_bands_object.x_array[i],temp[:,j],full_bands_object.colors_list[i][j],*args,**kwargs)

def full_bands_add_plot(full_bands_object,start_index,end_index,*args,**kwargs):
    full_bands_object.flatten()
    for i in range(full_bands_object.length()):
...
def full_bands_axes_sub_plot(subplot,full_bands_object):
    ...
    subplot.set_xlim(full_bands_object.x_ticks[0],full_bands_object.x_ticks[-1])
...
def flattened_full_bands_axes(flattened_full_bands_obj):
    ax = plt.axes()
    ax.xaxis.grid(True)

    plt.xlim(flattened_full_bands_obj.x_array[0],flattened_full_bands_obj.x_array[-1])
